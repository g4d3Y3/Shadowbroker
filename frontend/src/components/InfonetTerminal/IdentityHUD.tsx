    ...
    <div className="absolute bottom-4 right-4 z-[3] flex flex-col items-end">
      {isExpanded && (
        <div className="mb-2 w-64 bg-[#0a0a0a] border border-gray-800 p-3 shadow-[0_0_20px_rgba(6,182,212,0.1)]">
          <div className="flex justify-between items-center mb-3 border-b border-gray-800 pb-2">
            <span className="text-sm text-gray-500 uppercase tracking-widest font-bold">{t('hud_title')}</span>
          ...
          <div className="space-y-2">
            {(Object.keys(DOMAINS) as Domain[]).map((key) => {
              ...
              const visKey = key === 'ROOT' ? 'hud_never_public'
                : key === 'TRANSPORT' ? 'hud_public'
                : key === 'DM_ALIAS' ? 'hud_semi_obfuscated'
                : key === 'COIN' ? 'hud_never_linked'
                : 'hud_gate_scoped';
              return (
                <div key={key} className={`p-2 border ${isActive ? 'border-cyan-500 bg-cyan-500/5' : 'border-gray-800 bg-gray-900/20'} flex items-center justify-between group`}>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className={`text-sm font-bold tracking-tighter ${isActive ? 'text-white' : 'text-gray-500'}`}>{t('hud_' + key.toLowerCase().replace('_alias', '_alias').replace('_session', '_session').replace('_persona', '_persona'))}</p>
                      <p className="text-[12px] text-gray-600 uppercase">{t(visKey)}</p>
                    </div>
                  </div>
                  {isActive && (
                    <div className="flex gap-1">
                      <button title={t('hud_rotate')} className="p-1 text-gray-600 hover:text-cyan-400 transition-colors">
                        <RefreshCw size={10} />
                      </button>
                      <button title={t('hud_purge')} className="p-1 text-gray-600 hover:text-red-400 transition-colors">
                        <Trash2 size={10} />
                      </button>
                    </div>
                  )}
      ...
      >
        <div className="flex flex-col items-end">
          <span className="text-sm text-gray-500 uppercase tracking-widest leading-none mb-1">{t('hud_active_domain')}</span>
